...
`app/cjson/patches/0002-exact-integer-hooks.patch` により、数値項目の `valuestring` が非 `NULL` になる場合があります (正確な整数値を保持するための副作用)。  
数値項目から文字列を取得する際は、型を検査する `cJSON_GetStringValue` を使用してください。`valuestring` を直接参照しないでください。
